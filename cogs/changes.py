...
class Changes(commands.Cog):
    ...
    async def show_alliance_history_menu(self, interaction: discord.Interaction):
        try:
            embed = discord.Embed(
                ...
                description=(
                    f"**{_('available_operations', 'CHANGES')}**\n"
                    f"{_('separator', 'CHANGES')}\n"
                    f"🔥 **{_('furnace_changes', 'CHANGES')}**\n"
                    f"└ {_('view_furnace_level_changes', 'CHANGES')}\n\n"
                    f"📝 **{_('nickname_changes', 'CHANGES')}**\n"
                    f"└ {_('view_nickname_history', 'CHANGES')}\n"
                    f"{_('separator', 'CHANGES')}"
                ),
        ...
        try:
            ...
            if guild:
                ...
                if member:
                    ...
                    print(f"[changes.get_admin_alliances] 用戶: {member.display_name}")
                    print(f"[changes.get_admin_alliances] Manager: {has_manager_role}, Admin: {has_admin_role}")
                    ...
                    if has_manager_role or has_admin_role:
                        with sqlite3.connect('db/alliance.sqlite') as alliance_db:
                            ...
                            print(f"[changes.get_admin_alliances] 找到 {len(alliances)} 個聯盟")
            ...
            with sqlite3.connect('db/settings.sqlite') as settings_db:
                ...
                if not admin_result:
                    print(f"[changes.get_admin_alliances] User {user_id} 沒有在 admin 表中，也沒有 Discord 角色")
                    return [], [], False
    ...
    async def show_furnace_history(self, interaction: discord.Interaction, fid: int):
        try:
            ...
            await interaction.followup.send(
                _("error_displaying_furnace_history", "ERRORS"),
                ephemeral=True
            )
    ...
    async def show_nickname_history(self, interaction: discord.Interaction, fid: int):
        try:
            ...
            await interaction.followup.send(
                _("error_displaying_nickname_history", "ERRORS"),
                ephemeral=True
            )
        ...
        try:
            ...
            if not members:
                await interaction.response.send_message(
                    _("no_members_found_in_alliance", "ERRORS"),
                    ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                _("error_displaying_member_list", "ERRORS"),
                ephemeral=True
            )
    ...
    async def show_recent_changes(self, interaction: discord.Interaction, alliance_name: str, hours: int):
        try:
            ...
            await interaction.followup.send(
                _("error_showing_recent_changes", "ERRORS"),
                ephemeral=True
            )
...
class HistoryView(discord.ui.View):
    ...
    @discord.ui.button(
        label=_('furnace_changes', 'CHANGES'),
        emoji="🔥",
    ...
    async def furnace_changes_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if not admin_info:
                await interaction.response.send_message(
                    _("no_permission_to_perform_action", "PERMISSIONS"),
                    ephemeral=True
                )
                return
            ...
            if not available_alliances[0]:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        title=_("no_available_alliance", "ERRORS"),
                        description=_("no_alliance_found_access", "ERRORS"),
            ...
            if special_alliances:
                ...
                special_alliance_text += "━━━━━━━━━━━━━━━━━━━━━━\n"
                for alliance_id_item, name in special_alliances:
            ...
            select_embed = discord.Embed(
                ...
                description=(
                    ...
                    "**Permission Details**\n"
                    "━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"👤 **{_('access_level', 'PERMISSIONS')}:** `{_('global_admin', 'PERMISSIONS') if admin_info[1] == 1 else _('server_admin', 'PERMISSIONS')}`\n"
            ...
            view = AllianceSelectForHistoryView(alliances_with_counts, self.cog, page=0, context="furnace_history")

            async def alliance_callback(select_interaction: discord.Interaction):
                print(f"[DEBUG] furnace_changes alliance_callback triggered")
                try:
                    alliance_id = int(view.current_select.values[0])
                    print(f"[DEBUG] Selected alliance_id: {alliance_id}")
                    print(f"[DEBUG] Calling member_callback...")
                    await self.member_callback(select_interaction, alliance_id)
                    print(f"[DEBUG] member_callback completed")
                except Exception as e:
                    print(f"[ERROR] Error in alliance selection: {e}")
                    import traceback
                    traceback.print_exc()
                    try:
                        if not select_interaction.response.is_done():
                            await select_interaction.response.send_message(
                                _("error_processing_selection", "ERRORS"),
                                ephemeral=True
                            )
                        else:
            ...
            print(f"[DEBUG] Setting callback for furnace_changes view")
            view.callback = alliance_callback
            print(f"[DEBUG] Callback set: {view.callback is not None}")
            
            # 使用 edit_message 因為這是按鈕點擊
            try:
                await interaction.response.edit_message(
                    embed=select_embed,
                    view=view
            ...
            try:
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        _("error_processing_request", "ERRORS"),
                        ephemeral=True
                    )
                else:
                    await interaction.followup.send(
    ...
    async def member_callback(self, interaction: discord.Interaction, alliance_id: int):
        try:
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    _("error_showing_member_list", "ERRORS"),
                    ephemeral=True
                )
            else:
                await interaction.followup.send(
    ...
    @discord.ui.button(
        label=_('nickname_changes', 'CHANGES'),
        emoji="📝",
        style=discord.ButtonStyle.primary,
    ...
    async def nickname_changes_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            view = AllianceSelectForHistoryView(alliances_with_counts, self.cog, page=0, context="nickname_history")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if hours < 1 or hours > 24:
                await interaction.response.send_message(
                    _("please_enter_number_between", "ERRORS"),
                    ephemeral=True
                )
                return
...
class AllianceSelectForHistoryView(discord.ui.View):
    """專用於聯盟歷史的聯盟選擇視圖"""
    def __init__(self, alliances_with_counts, cog, page=0, context="furnace_history"):
    ...
    def update_select_menu(self):
        ...
        async def select_callback(interaction: discord.Interaction):
            print(f"[DEBUG] AllianceSelectForHistoryView select_callback triggered")
            print(f"[DEBUG] Selected value: {select.values[0] if select.values else 'None'}")
            print(f"[DEBUG] Has custom callback: {self.callback is not None}")
            try:
                if self.callback:
                    print(f"[DEBUG] Calling custom callback...")
                    await self.callback(interaction)
                    print(f"[DEBUG] Custom callback completed")
                else:
                    print(f"[DEBUG] No custom callback set!")
                    await interaction.response.send_message(
                        "❌ 錯誤：沒有設定回調函數",
                        ephemeral=True
                    )
            except Exception as e:
                print(f"[ERROR] Error in select_callback: {e}")
                import traceback
                traceback.print_exc()
                try:
                    if not interaction.response.is_done():
