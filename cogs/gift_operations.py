...
class GiftOperations(commands.Cog):
    def __init__(self, bot):
        ...
        self._last_retry_check_date = None  # 追蹤上次重試檢查日期
        ...
        # 2CAPTCHA Service Initialization
        try:
            # Try to initialize 2CAPTCHA service using environment variable
            twocaptcha_api_key = os.getenv("TWOCAPTCHA_API_KEY")
            if twocaptcha_api_key and twocaptcha_api_key.strip():
                self.two_captcha_service = TwoCaptchaService(twocaptcha_api_key)
                self.logger.info("2CAPTCHA service initialized successfully.")
            else:
                self.two_captcha_service = None
                self.logger.info("2CAPTCHA service not initialized - TWOCAPTCHA_API_KEY not found in environment variables.")
        except Exception as e:
            self.logger.exception(f"Error initializing 2CAPTCHA service: {e}")
            self.two_captcha_service = None
    ...
    async def _process_queue_item(self, queue_item):
        ...
        if self.cursor.fetchone():
            ...
            # 即使禮品碼已存在，仍然觸發自動兌換（與手動新增一致）
            self.logger.info(f"[_process_queue_item] Triggering auto-use for existing code '{giftcode}'")
    ...
    async def _send_existing_code_response(self, message, giftcode, channel):
        ...
        reply_embed.description = (
            ...
            f"🔄 **Action:** Starting automatic redemption for all enabled alliances.\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
        )
    ...
    async def start_automatic_redemption(self, giftcode):
        """Start automatic redemption for all enabled alliances."""
        try:
            self.logger.info(f"[start_automatic_redemption] Called for gift code: {giftcode}")
            
            # 獲取所有啟用自動兌換的聯盟
            self.cursor.execute("SELECT alliance_id FROM giftcodecontrol WHERE status = 1")
            auto_alliances = self.cursor.fetchall()
            
            self.logger.info(f"[start_automatic_redemption] Found {len(auto_alliances)} enabled alliances")
            
            if not auto_alliances:
                self.logger.warning(f"[start_automatic_redemption] No enabled alliances found")
                return "⚠️ 沒有啟用自動兌換的聯盟"
            ...
            for alliance in auto_alliances:
                alliance_id = alliance[0]
                try:
                    self.logger.info(f"[start_automatic_redemption] Adding to queue: alliance {alliance_id}, code {giftcode}")
                    await self.add_to_validation_queue(
                        giftcode=giftcode,
                    ...
                    self.logger.error(f"Error queuing redemption for alliance {alliance_id}: {e}")
            
            # 計算預估時間（每個聯盟平均 2-5 分鐘，取決於成員數量）
            estimated_time = redeemed_count * 3  # 平均每個聯盟 3 分鐘
            result_msg = f"🔄 已為 {redeemed_count} 個聯盟排程兌換\n⏱️ 預計 {estimated_time} 分鐘內完成（依成員數量而定）\n📢 兌換結果將即時發送到設定的頻道"
            self.logger.info(f"[start_automatic_redemption] Completed: {result_msg}")
            return result_msg
            
        except Exception as e:
            self.logger.exception(f"Error in start_automatic_redemption: {e}")
            return "❌ 啟動兌換時發生錯誤"
    ...
    async def on_ready(self):
        ...
        try:
            ...
            # Start auto-retry redemption loop
            if not self.auto_retry_redemption_loop.is_running():
                self.auto_retry_redemption_loop.start()
                self.logger.info("Started auto-retry redemption loop (12 hour interval)")
    ...
    async def _try_2captcha_fallback(self, image_bytes, captcha_image_base64):
        """
        當OCR四次失敗後，使用2CAPTCHA作為回退機制
        
        Args:
            image_bytes: 原始驗證碼圖片的bytes
            captcha_image_base64: Base64編碼的驗證碼圖片
            
        Returns:
            tuple: (captcha_code, success, method)
        """
        try:
            if not self.two_captcha_service:
                return None, False, "2CAPTCHA_NOT_CONFIGURED"
            
            if not self.two_captcha_service.is_configured():
                return None, False, "2CAPTCHA_NOT_CONFIGURED"
            
            self.logger.info("嘗試使用2CAPTCHA解決驗證碼...")
            ...
            try:
                ...
                self.logger.error(f"準備2CAPTCHA base64數據失敗: {e}")
                return None, False, "2CAPTCHA_DATA_ERROR"
            
            # 使用2CAPTCHA解碼
            success, result, error = await self.two_captcha_service.solve_captcha(img_b64_data)
            
            if success and result:
                self.logger.info(f"2CAPTCHA解碼成功: {result}")
                return result, True, "2CAPTCHA"
            else:
                self.logger.warning(f"2CAPTCHA解碼失敗: {error}")
                return None, False, f"2CAPTCHA_ERROR: {error}"
                
        except Exception as e:
            self.logger.exception(f"2CAPTCHA回退過程中發生錯誤: {e}")
            return None, False, f"2CAPTCHA_EXCEPTION: {str(e)}"
        ...
        for attempt in range(max_ocr_attempts):
            ...
            if not success:
                ...
                if attempt == max_ocr_attempts - 1:
                    # 第四次嘗試失敗，嘗試使用2CAPTCHA
                    if self.two_captcha_service and self.two_captcha_service.is_configured():
                        self.logger.info(f"GiftOps: OCR failed four times for FID {player_id}, attempting 2CAPTCHA fallback")
                        captcha_code, captcha_success, fallback_method = await self._try_2captcha_fallback(
                            image_bytes, captcha_image_base64)
                        
                        if captcha_success and captcha_code:
                            self.logger.info(f"GiftOps: 2CAPTCHA solved for {player_id}: {captcha_code}")
                            ...
                            if is_captcha_error:
                                ...
                                return "CAPTCHA_INVALID", image_bytes, captcha_code, fallback_method
                            else:
                                self.processing_stats["server_validation_success"] += 1
                            
                            # Return the appropriate status based on response
                            if msg == "SUCCESS":
                                status = "SUCCESS"
                            ...
                            return status, image_bytes, captcha_code, fallback_method
                        else:
                            self.logger.info(f"GiftOps: 2CAPTCHA also failed for FID {player_id}")
                            return "MAX_CAPTCHA_ATTEMPTS_REACHED", image_bytes, None, "2CAPTCHA_FAILED"
                    else:
                        self.logger.info(f"GiftOps: 2CAPTCHA not configured, giving up after 4 OCR attempts for FID {player_id}")
                        return "MAX_CAPTCHA_ATTEMPTS_REACHED", image_bytes, None, None
                continue
    ...
    async def auto_retry_redemption_loop(self):
        ...
        try:
            ...
            """)
            valid_codes = [row[0] for row in self.cursor.fetchall()]
            
            if not valid_codes:
                self.logger.info("[自動重試] 沒有找到有效的禮品碼")
                return
            
            self.logger.info(f"[自動重試] 找到 {len(valid_codes)} 個有效禮品碼需要檢查")
            
            # 2. 獲取所有啟用自動兌換的聯盟
            self.cursor.execute("SELECT alliance_id FROM giftcodecontrol WHERE status = 1")
            enabled_alliances = [row[0] for row in self.cursor.fetchall()]
            
            if not enabled_alliances:
                self.logger.info("[自動重試] 沒有找到啟用的聯盟")
                return
            
            self.logger.info(f"[自動重試] 檢查 {len(enabled_alliances)} 個已啟用的聯盟")
            
            retry_count = 0
            total_unredeemed = 0
            
            # 3. 對每個聯盟和每個禮品碼檢查
            for alliance_id in enabled_alliances:
                # 獲取聯盟名稱
                ...
                if not members:
                    self.logger.info(f"[自動重試] 聯盟 {alliance_name} 沒有成員")
                    continue
                
                self.logger.info(f"[自動重試] 檢查聯盟 {alliance_name}，共 {len(members)} 名成員")
                ...
                for giftcode in valid_codes:
                    ...
                    for fid, nickname in members:
                        ...
                        if not result:
                            # 完全沒有兌換記錄
                            unredeemed_fids.append((fid, nickname, "無記錄"))
                        elif result[0] not in ["SUCCESS", "RECEIVED", "SAME TYPE EXCHANGE"]:
                            # 有記錄但不是成功狀態
                            unredeemed_fids.append((fid, nickname, result[0]))
                    
                    # 如果有未兌換的FID，排程重試
                    if unredeemed_fids:
                        total_unredeemed += len(unredeemed_fids)
                        self.logger.info(f"[自動重試] 禮品碼 '{giftcode}' 在聯盟 {alliance_name}: {len(unredeemed_fids)}/{len(members)} 需要重試")
                        
                        # 記錄前5個需要重試的FID（用於日誌）
                        sample_fids = unredeemed_fids[:5]
                        sample_info = ", ".join([f"{fid}({status})" for fid, _, status in sample_fids])
                        self.logger.info(f"[自動重試] 樣本 FID: {sample_info}{'...' if len(unredeemed_fids) > 5 else ''}")
                        ...
                        self.logger.info(f"[自動重試] 已排程重試: {giftcode} for {alliance_name}")
            ...
            # 使用相同時區的時間計算
            taiwan_tz = pytz.timezone('Asia/Taipei')
            loop_end_time = datetime.now(taiwan_tz)
            duration = loop_end_time - loop_start_time
            self.logger.info(f"[自動重試] 完成！已排程 {retry_count} 個重試操作，共 {total_unredeemed} 個未兌換 FID。耗時: {duration}\n")
            
        except Exception as e:
            self.logger.exception(f"[自動重試] 自動重試循環發生錯誤: {e}")
            import traceback
            self.logger.error(f"[自動重試] Traceback: {traceback.format_exc()}")

    @auto_retry_redemption_loop.before_loop
    async def before_auto_retry_loop(self):
        """等待機器人準備好，並在啟動時立即執行一次檢查"""
        from datetime import datetime, timedelta
        import pytz
        
        self.logger.info("[自動重試] 等待機器人準備好後啟動自動重試循環...")
        await self.bot.wait_until_ready()
        ...
        if current_hour < 8:
            next_hour = 8
            wait_time = (8 - current_hour) * 3600 - now_taiwan.minute * 60 - now_taiwan.second
        elif current_hour < 20:
            next_hour = 20
            wait_time = (20 - current_hour) * 3600 - now_taiwan.minute * 60 - now_taiwan.second
        else:
            next_hour = 8
            wait_time = (24 - current_hour + 8) * 3600 - now_taiwan.minute * 60 - now_taiwan.second
        ...
        try:
            if interaction:
                ...
                await interaction.followup.send(
                    "🔄 開始手動重試檢查...\n這可能需要幾分鐘，請查看日誌以獲取詳細資訊。",
                    ephemeral=True
                )
            
            self.logger.info("[手動重試] 管理員觸發手動重試檢查")
            
            # 執行與自動循環相同的邏輯
            await self.auto_retry_redemption_loop()
            
            if interaction:
                await interaction.followup.send("✅ 手動重試檢查完成！", ephemeral=True)
        except Exception as e:
            self.logger.exception(f"[手動重試] 手動重試檢查發生錯誤: {e}")
            if interaction:
                await interaction.followup.send(f"❌ 重試檢查失敗: {str(e)}", ephemeral=True)
    ...
    async def show_gift_menu(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"{_('please_select_operation', 'GIFT_CODE')}\n\n"
                    f"**{_('available_operations', 'GIFT_CODE')}**\n"
    ...
    async def use_giftcode_for_alliance(self, alliance_id, giftcode):
        ...
        try:
            ...
            if not name_result:
                self.logger.error(f"GiftOps: Could not find alliance {alliance_id}.")
                return False
            ...
            try:
                ...
                with sqlite3.connect('db/settings.sqlite') as settings_conn:
                    settings_cursor = settings_conn.cursor()
                    settings_cursor.execute("SELECT global_gift_code_channel FROM botsettings WHERE id = 1")
            ...
            if not channel_id:
                try:
                    ...
                    if channel_result and channel_result[0]:
                        ...
                        channel_source = "alliance"
                        self.logger.info(f"GiftOps: Using alliance-specific channel {channel_id} for alliance {alliance_name}")
                except Exception as e:
            ...
            try:
                ...
                self.logger.error(f"GiftOps: Error getting channel object for ID {channel_id}: {e}")
                return False
            ...
            def update_embed_description(include_errors=False):
                ...
                base_description = (
                    f"{_('status_for_alliance', 'GIFT_CODE').format(name=alliance_name)}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"{_('total_members', 'GIFT_CODE').format(count=total_members)}\n"
                    f"{_('success_count', 'GIFT_CODE').format(count=success_count)}\n"
                    f"{_('already_redeemed', 'GIFT_CODE').format(count=received_count)}\n"
                    f"{_('retrying_count', 'GIFT_CODE').format(count=len(retry_queue))}\n"
                    f"{_('failed_count', 'GIFT_CODE').format(count=failed_count)}\n"
                    f"{_('processed_progress', 'GIFT_CODE').format(processed=processed_count, total=total_members)}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                )
                ...
                if include_errors and failed_count > 0:
                    ...
                    if non_success_errors:
                        ...
                        error_descriptions = {
                            "TOO_POOR_SPEND_MORE": _("error_vip_too_low", "GIFT_CODE"),
                            "TOO_SMALL_SPEND_MORE": _("error_furnace_too_low", "GIFT_CODE"),
                            ...
                            "SIGN_ERROR": _("error_sign_error", "GIFT_CODE"),
                            "ERROR": _("error_general", "GIFT_CODE"),
                            "UNKNOWN_API_RESPONSE": _("error_unknown_response", "GIFT_CODE")
                        }
                        
                        base_description += f"\n{_('error_breakdown', 'GIFT_CODE')}\n"
                        ...
                        for error_type, count in sorted(non_success_errors.items(), key=lambda x: x[1], reverse=True):
                            if error_type in error_descriptions:
                                ...
                                base_description += f"❗ **{count}** 位成員因狀態而失敗: {error_type}\n"
...
class CreateGiftCodeModal(discord.ui.Modal):
    def __init__(self, cog):
        super().__init__(title=_('add_gift_code', 'GIFT_CODE'))
        self.cog = cog
        
        self.giftcode = discord.ui.TextInput(
            label=_('enter_gift_code', 'GIFT_CODE'),
            placeholder=_('gift_code_placeholder', 'GIFT_CODE'),
            required=True,
            min_length=4,
            max_length=20
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        final_embed = discord.Embed(title=f"🎁 {_('gift_code_creation_result', 'GIFT_CODE')}")
        ...
        if self.cog.cursor.fetchone():
            ...
            # 即使代碼已存在，仍然啟動自動兌換
            redemption_status = await self.cog.start_automatic_redemption(code)
            logger.info(f"[CreateGiftCodeModal] Started redemption for existing code {code}: {redemption_status}")
            
            final_embed.title = f"ℹ️ {_('gift_code_already_exists', 'GIFT_CODE')}"
            final_embed.description = (
                f"**{_('gift_code_details', 'GIFT_CODE')}**\n━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🎁 **{_('gift_code_placeholder', 'GIFT_CODE').split('：')[0]}：** `{code}`\n"
                f"✅ **{_('status', 'GIFT_CODE')}：** Code already exists in database.\n"
                f"🔄 **{_('兌換', 'GIFT_CODE')}：** {redemption_status}\n"
                f"━━━━━━━━━━━━━━━━━━━━━━\n"
            )
            ...
            if is_valid: # Valid code - send to API and add to DB, then start redemption
                ...
                # Start automatic redemption process
                redemption_status = await self.cog.start_automatic_redemption(code)
                
                final_embed.title = "✅ 禮品碼新增並開始兌換"
                final_embed.description = (
                    f"**禮品碼詳情**\n━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"🎁 **禮品碼:** `{code}`\n"
                    f"✅ **狀態:** {validation_msg}\n"
                    f"📝 **操作:** 已新增到資料庫並發送到 API\n"
                    f"🔄 **兌換:** {redemption_status}\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                )
...
class SimplifiedGiftView(discord.ui.View):
    def __init__(self, cog):
        super().__init__(timeout=7200)
        self.cog = cog

    @discord.ui.button(
        label=_('add_gift_code', 'GIFT_CODE'),
        style=discord.ButtonStyle.success,
        custom_id="create_gift_code",
        emoji="🎁",
        row=0
    )
