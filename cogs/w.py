...
class WCommand(commands.Cog):
    ...
    @discord.app_commands.command(name='w', description=_('fetch_user_info_description', 'USER_QUERY'))
                    ...
                    async with session.post(url, headers=headers, data=form, ssl=ssl_context) as response:
                        if response.status == 200:
                            ...
                            embed = discord.Embed(
                                title=f"👤 {nickname}",
                                description=(
                                    f"{_('user_info_separator', 'USER_QUERY')} \n"
                                    f"**🆔 {_('id', 'USER_QUERY')}:** `{fid_value}`\n"
                                    f"**🔥 {_('furnace_level', 'USER_QUERY')}:** `{stove_level_name}`\n"
                                    f"**🌍 {_('state', 'USER_QUERY')}:** `{kid}`\n"
                                    f"{_('user_info_separator', 'USER_QUERY')}\n"
                                ),
                            ...
                            if alliance_info:
                                embed.description += f"**🏰 {_('alliance', 'USER_QUERY')}:** `{alliance_info[0]}`\n{_('user_info_separator', 'USER_QUERY')}\n"

                            registration_status = _('registered_on_list', 'USER_QUERY') if user_info else _('not_on_list', 'USER_QUERY')
                            ...
                            if attempt < max_retries - 1:
                                await interaction.followup.send(_('api_limit_reached', 'USER_QUERY'))
                                await asyncio.sleep(retry_delay)
            await interaction.followup.send(_('user_not_found_error', 'USER_QUERY').format(fid=fid))
            
        except Exception as e:
            print(f"An error occurred: {e}")
            await interaction.followup.send(_('error_fetching_user_info', 'USER_QUERY'))
