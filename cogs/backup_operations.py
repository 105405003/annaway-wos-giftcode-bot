...
class BackupView(discord.ui.View):
    ...
    @discord.ui.button(label=_("main_menu", "GENERAL"), emoji="🏠", style=discord.ButtonStyle.secondary, row=1)
...
class BackupManageView(discord.ui.View):
    ...
    @discord.ui.button(label="清理舊備份", emoji="🧹", style=discord.ButtonStyle.secondary)
    async def clean_backups(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            ...
            description=(
                f"**刪除的檔案：**\n"
                f"• 手動備份：{manual_removed}\n"
                f"• 自動備份：{auto_removed}\n\n"
                f"**保留政策：**\n"
                f"• 保留最後 2 個手動備份\n"
                f"• 保留最後 2 個自動備份\n\n"
                f"**目前可用空間：** {space_info['free_mb']:.1f} MB" if space_info else ""
            ),
            color=discord.Color.green()
        )
...
class BackupPasswordModal(discord.ui.Modal, title="設定備份密碼"):
    def __init__(self, cog):
        super().__init__()
        self.cog = cog
